...
async function initializeDatabase() {
  ...
  try {
    ...
    const acmeTenant = (await query(`SELECT id FROM tenants WHERE slug = 'acme'`)).rows[0];
    const globexTenant = (await query(`SELECT id FROM tenants WHERE slug = 'globex'`)).rows[0];
...
app.post('/auth/login', async (c) => {
  ...
  try {
    ...
    const tenantResult = await query(
      `SELECT id, slug, name, plan FROM tenants WHERE id = $1`,
      [user.tenant_id]
    );
...
app.get('/notes', requireAuth(), async (c) => {
  ...
  try {
    const result = await query(
      `SELECT id, title, content, user_id, created_at, updated_at FROM notes WHERE tenant_id = $1 ORDER BY created_at DESC`,
      [auth.tenantId]
    );
...
app.get('/notes/:id', requireAuth(), async (c) => {
  ...
  try {
    const result = await query(
      `SELECT id, title, content, user_id, created_at, updated_at FROM notes WHERE id = $1 AND tenant_id = $2`,
      [id, auth.tenantId]
    );
...
app.post('/notes', requireAuth(), async (c) => {
  ...
  try {
    ...
    const result = await query(
      `INSERT INTO notes (tenant_id, user_id, title, content) VALUES ($1, $2, $3, $4) RETURNING id, title, content, user_id, created_at, updated_at`,
...
app.put('/notes/:id', requireAuth(), async (c) => {
  ...
  try {
    ...
    const result = await query(
      `UPDATE notes SET title = COALESCE($1, title), content = COALESCE($2, content), updated_at = NOW() WHERE id = $3 RETURNING id, title, content, user_id, created_at, updated_at`,
      [title, content, id]
    );
